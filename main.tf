...
resource "aws_subnet" "wp_public1_subnet" {
  ...
  availability_zone = "${data.aws_availability_zones.availables.names[0]}"
  tags {
    Name = "wp_public1"
  }
}
...
resource "aws_subnet" "wp_public2_subnet" {
  ...
  map_public_ip_on_launch = true
  availability_zone = "${data.aws_availability_zones.availables.names[1]}"
  tags {
    Name = "wp_public2"
  }
}
...
resource "aws_subnet" "wp_rds3_subnet" {
  ...
  map_public_ip_on_launch = false
  availability_zone = "${data.aws_availability_zones.availables.names[2]}"
  tags {
    Name = "wp_rds3"
  }
}
...
resource "aws_security_group" "wp_dev_sg" {
  name = "wp_dev_sg"
  description = "Used for access to the dev instances"
  vpc_id = "${aws_vpc.wp_vpc.id}"
  #SSH
  ingress {
    from_port = 22
...
resource "aws_security_group" "wp_rds_sg" {
  ...
  vpc_id = "${aws_vpc.wp_vpc.id}"
  #SQL Access from public or private groups
  ingress {
    from_port = 3306
    to_port = 3306
    protocol = "tcp"
